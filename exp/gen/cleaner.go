...
func cleanTCPMiddlewares(element *dynamic.Configuration) {
	valMds := reflect.ValueOf(element.TCP.Middlewares)

	mdRoot := element.TCP.Middlewares["TCPMiddleware0"]
	valueMdRoot := reflect.ValueOf(mdRoot).Elem()
	...
	for i, fieldName := range mdFieldNames {
		v := reflect.New(reflect.TypeOf(dynamic.TCPMiddleware{}))
		v.Elem().FieldByName(fieldName).Set(valueMdRoot.FieldByName(fieldName))

		valMds.SetMapIndex(reflect.ValueOf(fmt.Sprintf("TCPMiddleware%.2d", i)), v)
	}

	delete(element.TCP.Middlewares, "TCPMiddleware0")
	delete(element.TCP.Middlewares, "TCPMiddleware1")
}
